...
def main():
    ...
    parser = argparse.ArgumentParser(description='Yoga Pose Classification in Video')
    parser.add_argument('-i', '--input', type=str, required=True, help='Input video file path')
    parser.add_argument('--display', action='store_true', help='Display the output video')
    ...
    # Démarrage de la capture vidéo à partir du fichier
    cap = cv2.VideoCapture(args.input)

    # Préparer l'enregistreur vidéo si nécessaire
    out = None
    if args.output:
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(args.output, fourcc, 20.0, (int(cap.get(3)), int(cap.get(4))))

    # Boucle pour la détection de pose en continu
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            break

        # Détecter la pose et classer
        frame, landmarks = detectPose(frame, pose, display=False)
        if landmarks:
            frame, pose_label = classifyPose(landmarks, frame, display=False)

        # Enregistrer ou afficher le résultat
        if args.output:
            out.write(frame)
        if args.display:
    ...
    # Relâcher la capture vidéo et fermer toutes les fenêtres
    cap.release()
    if args.output:
        out.release()
