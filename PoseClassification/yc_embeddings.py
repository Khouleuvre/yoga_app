...
class PoseClassifier(object):
    ...
    def __call__(self, pose_landmarks):
        ...
        end_time = time.time()
        print(f"Classification time: {end_time - start_time:.5f} seconds")
        return result
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pose-samples-folder",
        type=str,
        default="pose_samples",
        help="Path to the folder containing the pose samples",
    )
    parser.add_argument(
        "--file-extension",
        type=str,
        default="csv",
        help="File extension of the pose samples",
    )
    parser.add_argument(
        "--file-separator",
        type=str,
        default=",",
        help="File separator of the pose samples",
    )
    parser.add_argument(
        "--n-landmarks",
        type=int,
        default=33,
        help="Number of landmarks in the pose samples",
    )
    parser.add_argument(
        "--n-dimensions",
        type=int,
        default=3,
        help="Number of dimensions of the landmarks in the pose samples",
    )
    parser.add_argument(
        "--top-n-by-max-distance",
        type=int,
        default=30,
        help="Top N samples to filter by max distance",
    )
    parser.add_argument(
        "--top-n-by-mean-distance",
        type=int,
        default=10,
        help="Top N samples to filter by mean distance",
    )
    parser.add_argument(
        "--axes-weights",
        type=float,
        nargs="+",
        default=[1.0, 1.0, 0.2],
        help="Weights for each axis when calculating distances",
    )
    args = parser.parse_args()

    pose_classifier = PoseClassifier(
        pose_samples_folder=args.pose_samples_folder,
        pose_embedder=None,
        file_extension=args.file_extension,
        file_separator=args.file_separator,
        n_landmarks=args.n_landmarks,
        n_dimensions=args.n_dimensions,
        top_n_by_max_distance=args.top_n_by_max_distance,
        top_n_by_mean_distance=args.top_n_by_mean_distance,
        axes_weights=args.axes_weights,
    )
