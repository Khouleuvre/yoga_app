...
def classifyPose(landmarks, output_image, display=False):
    ...
    if (
        left_knee_angle > 150
        and left_knee_angle < 200
        or right_knee_angle > 150
        and right_knee_angle < 200
    ):
        # Check if the other leg is bended at the required angle.
        if (
            left_knee_angle > 305
            and left_knee_angle < 340
            or right_knee_angle > 20
            and right_knee_angle < 50
        ):
            # Specify the label of the pose that is tree pose.
            label = "Tree Pose"
...
def main():
    parser = argparse.ArgumentParser(
        description="Yoga Pose Classification in Image/Video/Webcam"
    )
    parser.add_argument(
        "-i",
        "--input",
        type=str,
        help="Input image/video file path, leave blank for webcam",
    )
    ...
    if args.output and not is_video_file:
        ...
        out = cv2.VideoWriter(
            args.output, fourcc, 20.0, (int(cap.get(3)), int(cap.get(4)))
        )
